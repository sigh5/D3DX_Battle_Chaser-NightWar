...
{
	//Model_EffectSoundVec.reserve(200);
}
...
void CSoundPlayer::Add_SoundEffect_Model(CModel * pModel, tag_SoundDesc & Desc)
{
	...
	tag_SoundDesc	ModelSoundDesc;
	
	ZeroMemory(&ModelSoundDesc, sizeof(ModelSoundDesc));
	memcpy(&ModelSoundDesc, &Desc, sizeof(ModelSoundDesc));
	Model_EffectSoundVec.push_back({ pModel, ModelSoundDesc });
}
